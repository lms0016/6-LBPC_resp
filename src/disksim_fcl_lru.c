...
void lru_insert1(struct cache_manager *c,struct lru_node *ln){  // fox
	struct lru_node *node;
	struct list_head *head;
	struct list_head *pos;
	head = &c->cm_head; 
	pos = head->next; // the first entry
	if (!list_empty(head)) { // list is not empty
		node = (struct lru_node *) list_entry(pos, struct lru_node, cn_list); // get ln to compare prize
		while (ln->cn_prize < node->cn_prize) { // make the list to be big to small, head->prev = LRU position = minPrize

			pos = pos->next;
			if (pos != head) {
				node = (struct lru_node *) list_entry(pos, struct lru_node, cn_list);
			} else 
				break;	
		}		
	} 
	list_add(&ln->cn_list, pos->prev); // if empty, add to list
	...
	c->cm_free--;
	c->cm_count++;
    //  lru_print_screen(c); // print cache content
...
void *lru_replace1(struct cache_manager *c, int watermark, int replace_type ){	 // fox
	struct list_head *remove_ptr;
	struct lru_node *victim = NULL;
	struct lru_node *active_ln = NULL;
	int free;
	switch ( replace_type ) {
		case FCL_REPLACE_DIRTY:
			//free = c->cm_dirty_size - c->cm_dirty_count;
	...
	if ( free < watermark + 1 ) {
		...
		if (victim != NULL ) {
			active_ln = fcl_lookup_active_list ( victim->cn_blkno ); 
			while (victim->cn_flag != 2 || active_ln != NULL) { // prevent evict wrong victim
				remove_ptr = remove_ptr->prev; // try evict next victim (next LRU pos)
				if (replace_type == FCL_REPLACE_DIRTY) {
					victim = list_entry ( remove_ptr, struct lru_node, cn_dirty_list );
				} else if (replace_type == FCL_REPLACE_CLEAN) {
					victim = list_entry ( remove_ptr, struct lru_node, cn_clean_list );
				} else if (replace_type == FCL_REPLACE_ANY) {
					victim = list_entry ( remove_ptr, struct lru_node, cn_list );
				}	
				active_ln = fcl_lookup_active_list ( victim->cn_blkno );
			} // end while	
...
void lru_print_screen ( struct cache_manager *c ) {  // fox
	printf("\n");
	struct list_head *head, *ptr;
	struct lru_node *ln;
	head = &c->cm_head;
	list_for_each( ptr, head ) {
		ln = (struct lru_node *) list_entry ( ptr, struct lru_node, cn_list );	
		printf (" blkno = %d, prize = %lf \n", ln->cn_blkno, ln->cn_prize );
	}	
	printf ( " lru count = %d \n", c->cm_count );

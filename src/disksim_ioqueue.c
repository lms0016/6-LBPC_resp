...
*/
   ...
   */
   if (tmp->reqcnt == 1) { // all in this if statement
     ioqueue_remove_from_subqueue(queue, tmp);
      trv = done;
...
// fox
    ...
    t_count=t_count+1;
	subqueue *temp_q = NULL;
	temp_q = &fore_q->base;
	if ( temp_q == queue ) {
		if (done->devno) { // SSD
			S_rTime += (simtime -tmp->iob_un.time);
			S_count++;
		} else { // HDD
			H_rTime += (simtime -tmp->iob_un.time);
			H_count++;	
		}
		if ( ++count%10000 == 0) {
			double S_avgr = 0.0;
			...
			if ( hcount ) {
				...
				H_avgr = (double) H_rTime/hcount;
				if(t_count%2==1)
				h_favg=(double) H_rTime/hcount;
				
			}
			if ( scount ) {
                if(t_count==1||t_count%2==0)
				S_avgr = (double) S_rTime/scount;
				if(t_count%2==1)
				s_favg=(double) S_rTime/scount;
			}
			//printf("\nSSD avgr: %.3f, avga: %.3f, count: %d\n", S_avgr, S_avga, S_count );
			//printf("HDD avgr: %.3f, avga: %.3f, count: %d\n", H_avgr, H_avga, H_count );
			if(t_count!=1&&t_count%2==1){
               diff=(h_favg-s_favg)*rate+(1-rate)*(H_avgr-S_avgr);      //SMALL NEW     
               printf("diff=%f\n",diff);  
            }
            if(t_count==1){
			   diff = S_avgr - H_avgr;
            ...
            if(t_count%2==0){
               diff=(H_avgr-S_avgr)*rate+(1-rate)*(h_favg-s_favg);      //BIG NEW   
               printf("diff=%f\n",diff);     
            }
			int j = 0;
			if ( diff < 0 ) { 
			...
			if ( diff > 50 )
				if ( diff > 100 )
					set_sequnit(128,j); // >100
				else 
					set_sequnit(64,j); // 50~100
			else 
				if ( diff > 5 )
					if ( diff > 25)
						set_sequnit(32,j); // 25~50
					else 
						set_sequnit(16,j); // 5~25			
				else 
					set_sequnit(8,j); // 0~5

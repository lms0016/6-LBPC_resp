...
//	int cn_ssd_sector;
	...
	double cn_time;
	double cn_prize; // fox: prize value
};
...
struct cache_manager{
 ...
 void *(*cache_replace1)(struct cache_manager *cache,int w, int dirty); // evofox
 ...
 void (*cache_insert1)(struct cache_manager *cache, struct lru_node *node); // evofox
...
#define CACHE_REPLACE1(c, w, d) c->cache_replace1((struct cache_manager *)c, w, d) // evofox
...
#define CACHE_INSERT1(c, p) c->cache_insert1((struct cache_manager *)c, p) // evofox
...
void lru_insert1(struct cache_manager *c,struct lru_node *ln); // evofox
void *lru_replace(struct cache_manager *c, int watermark, int dirty);	
void *lru_replace1(struct cache_manager *c, int watermark, int dirty); // evofox	
...
void lru_print_screen ( struct cache_manager *c ); // evofox

...
double baseprize = 0; // fox: base prize value
...
int _fcl_replace_cache ( ioreq_event *parent, int watermark, int replace_type ) {
	...
	remove_ln = CACHE_REPLACE1(fcl_cache_mgr, watermark, replace_type); // fox
	...
	if (remove_ln) { 
		baseprize = remove_ln->cn_prize; // fox: set victim's prize as base
	}
	...
	{ 
		...
		CACHE_INSERT1( fcl_cache_mgr, remove_ln ); // fox
...
void fcl_make_seq_req (ioreq_event *parent, int blkno) {
	...
	if ( parent->flags & READ ) {
		if ( !rhit ) { 
			...
			fcl_generate_child_request ( parent, HDD, blkno, READ, 2, 0);
		} else {
...
void fcl_make_normal_req (ioreq_event *parent, int blkno) {
	...
	CACHE_INSERT1(fcl_cache_mgr, ln); // fox

}


void fcl_make_normal_req1 (ioreq_event *parent, int blkno) {  // fox
	struct lru_node *ln = NULL;
	int hit = 0;
	int goHDD = 0; 
	double prize = parent->prize; 
	ln = CACHE_SEARCH(fcl_cache_mgr, blkno);
	if(ln){ // hit case 
		hit = 1;
		ln = CACHE_REMOVE(fcl_cache_mgr, ln);
		...
		if (fcl_cache_mgr->cm_free != 0 && prize > minP ) {  
			ln = fcl_replace_cache ( parent, blkno, NULL ); 
			ASSERT ( ln );
		} else {
			struct list_head *head = &fcl_cache_mgr->cm_head;
			struct list_head *pos = head->prev; 
			struct lru_node *minPrizeNode = (struct lru_node *) list_entry(pos, struct lru_node, cn_list); 
			if ( prize > minPrizeNode->cn_prize && prize > minP ) { // prize > minPrize
				ln = fcl_replace_cache( parent, blkno, NULL); 
				ASSERT( ln );
			} else {  
				goHDD = 1;
	...
	if (goHDD == 1) { 
		if ( parent->flags & READ) { 
			if (!hit) { 
				//fcl_generate_child_request(parent, HDD, blkno, READ, 0, 0 ); 
				fcl_generate_child_request( parent, HDD, blkno, READ, 2, 1 );
			}
		} else { 
			//fcl_generate_child_request( parent, HDD, blkno, WRITE, 1, 0); 
			fcl_generate_child_request( parent, HDD, blkno, WRITE, 1, 0 );
		...
		ln->cn_prize = parent->prize;
		CACHE_INSERT1(fcl_cache_mgr, ln); // fox
	}
} // end fcl_make_normal_req1
...
void fcl_make_request ( ioreq_event *parent, int blkno ) {
	switch ( parent->tempint1 ) {
		...
		*/
		if ( sd_is_seq_io(blkno)){ // fox
			parent->devno = 0;
			fcl_make_seq_req( parent, blkno );
		}else{
			parent->devno = 1; 
			fcl_make_normal_req1( parent, blkno ); // fox
		}
			break;
...
/*
	...
	ioreq_event *temp[16];
	int i = 0, j = 0, first_key = 0, first_length = 0, last_length = 0;
	if (op_type == FCL_OPERATION_NORMAL) {
		if (parent->blkno % blk_mod == 0) { // align with blkno_start
				if ( parent->bcount > blk_mod ) { // bigger than 1 blk size
					j = parent->bcount / blk_mod;
					last_length = parent->bcount % blk_mod;
					temp[0] = (ioreq_event *)fcl_create_parent( parent->blkno, blk_mod, simtime, parent->flags, parent->devno );  
					fcl_parent_init(temp[0]);
					temp[0]->tempint1 = op_type;
					if ( (j == 0) && (last_length!=0) ) 
						temp[i]->bcount = last_length;
					temp[0]->prize = calculate_prize( temp[0]->blkno, baseprize, temp[0]->bcount, temp[0]->flags, parent->blkno);
					fcl_add_new_request(temp[0], op_type);
					for ( i = 1; i < j+1; i++ ) {
						temp[i] = (ioreq_event *)fcl_create_parent( parent->blkno+(i*blk_mod), blk_mod, simtime, parent->flags, parent->devno );  
						fcl_parent_init(temp[i]);
						temp[i]->tempint1 = op_type;
						if ( (i == j) && (last_length!=0) ) 
							temp[i]->bcount = last_length;
						temp[i]->prize = calculate_prize( temp[i]->blkno, baseprize, temp[i]->bcount, temp[i]->flags, temp[i]->blkno);
						fcl_add_new_request(temp[i], op_type);
					} // end for
				} else { // less than 1 blk size
					j = 0;
					fcl_parent_init ( parent );
					parent->tempint1 = op_type;
					if ( op_type == FCL_OPERATION_NORMAL )  
						parent->prize = calculate_prize( parent->blkno, baseprize, parent->bcount, parent->flags, parent->blkno);
					fcl_add_new_request( parent, op_type ); 
				} 
		} else { 

			first_key = parent->blkno/blk_mod * blk_mod;

			if ( (parent->blkno + parent->bcount-1) > (first_key + blk_mod-1) ) { // div into two node
				int t_length = parent->bcount; // orig length
				first_length = first_key + blk_mod - parent->blkno;  // first node's length

				temp[0] = (ioreq_event *)fcl_create_parent(parent->blkno, first_length, simtime, parent->flags, parent->devno); 
				fcl_parent_init(temp[0]);
				temp[0]->tempint1 = op_type;
				temp[0]->prize = calculate_prize( first_key, baseprize, temp[i]->bcount, temp[i]->flags, parent->blkno);
				fcl_add_new_request(temp[0], op_type);
				
				t_length -= first_length;
				j = t_length / blk_mod;
				last_length = t_length % blk_mod;
				for ( i = 1; i < j+1; i++ ) {
					temp[i] = (ioreq_event *)fcl_create_parent( first_key+(i*blk_mod), blk_mod, simtime, parent->flags, parent->devno );  
					fcl_parent_init(temp[i]);
					temp[i]->tempint1 = op_type;
					if ( (i == j) && (last_length!=0) ) 
						temp[i]->bcount = last_length;
				...
				parent->tempint1 = op_type;
				if ( op_type == FCL_OPERATION_NORMAL )  
					parent->prize = calculate_prize( first_key, baseprize, parent->bcount, parent->flags, parent->blkno);
				fcl_add_new_request( parent, op_type ); 
			}
		}

	} else { // not FCL_OPERATION_NORMAL
		fcl_parent_init ( parent );
...
void fcl_request_arrive (ioreq_event *parent) {
	...
	if ( ++debug_arrive % 100000 == 0 ) {
		printf ( "debug_arrive:%d, base:%.3lf, FCL Req Arrive time = %.2f, blkno = %d, bcount = %d, flags = %d, devno = %d, fqueue = %d, bqueue = %d \n", 
				debug_arrive, baseprize, simtime, parent->blkno, parent->bcount, parent->flags, parent->devno, ioqueue_get_number_in_queue ( fcl_fore_q ),
				ioqueue_get_number_in_queue ( fcl_back_q ) );
		...
		printf ( " FCL Written = %.2fMB, Read = %.2fMB \n", (double)fcl_stat->fstat_io_write_pages/256, (double)fcl_stat->fstat_io_read_pages/256);
		

		printf(" cm_free = %.2fMB\n", (double) (fcl_cache_mgr->cm_free)/256); 
		printf(" caching space = %.2fMB\n", (double) (fcl_cache_mgr->cm_size)/256); 
		printf(" LRU list count: %d (%.2fMB)\n", (fcl_cache_mgr->cm_dirty_count+fcl_cache_mgr->cm_clean_count), (double)(fcl_cache_mgr->cm_dirty_count+fcl_cache_mgr->cm_clean_count)/256);
...
void fcl_remove_active_block_in_active_list (int blkno ) {
	...
	if ( ln ) {
		...
		if ( ll_get_size ( (listnode *)ln->cn_temp2 )){
			...
			//printf (" It has blocking child requests of some parents, parent blkno=%d \n",
			//		parent->blkno);
...
void fcl_exit () {
	printf("nodeCnt = %d\n", get_nodeCnt() ); 
	fprintf(outputfile, "nodeCnt = %d\n", get_nodeCnt());

//	lru_print_screen(fcl_cache_mgr); // fox: print cache info
